...
typedef struct 
	{
		...
		void reset()
		{
			for(int i=0;i<MAXJ;i++) {
				...
				genPt[i]=-99; genEta[i]=-99; genPhi[i]=-99; JECUnc[i]=-99; ptRaw[i]=-99.; ptLeadTrack[i]=-99.; puJetIdL[i]=-99; puJetIdM[i]=-99; puJetIdT[i]=-99; puJetIdMva[i]=-99; selectedTauDR[i] = -99.; charge[i]=-99; jetArea[i]=-99;
			}
		}
		float pt[MAXJ];
		float eta[MAXJ];
...
int main(int argc, char* argv[]) 
{
	...
	_outTree->Branch("isMostlyGarbage"         ,  &isMostlyGarbage   ,   "isMostlyGarbage/I");
	
	
	float allTau_pt, allTau_eta, allTau_phi, allTau_mass;
	int allTau_byLooseCombinedIsolationDeltaBetaCorr,allTau_againstMuonTight,allTau_againstElectronLoose,allTau_decayModeFinding,allTau_againstElectronMedium,allTau_againstElectronMVA;
	int allTau_NsignalPFChargedHadrCands, allTau_NsignalPFGammaCands; 
	float allTau_leadPFChargedHadrCandPt, allTau_byLooseIsolation, allTau_byMediumIsolation, allTau_byTightIsolation;
	float allTau_met, allTau_metSig, allTau_metPhi;
	float allTau_mc_pt, allTau_mc_eta, allTau_mc_phi;
	float allTau_nearestJetDR, allTau_nearestJetPt;
	
	TTree* _tauTree = new TTree("tauTree","tauTree");
	
	_tauTree->Branch("mass",&allTau_mass ,"mass/F");
	_tauTree->Branch("pt",&allTau_pt ,"pt/F");
	_tauTree->Branch("eta",&allTau_eta ,"eta");
	_tauTree->Branch("phi",&allTau_phi ,"phi/F");
	_tauTree->Branch("decayModeFinding",&allTau_decayModeFinding,"decayModeFinding/I");
	_tauTree->Branch("byLooseCombinedIsolationDeltaBetaCorr",&allTau_byLooseCombinedIsolationDeltaBetaCorr,"byLooseCombinedIsolationDeltaBetaCorr/I");
	_tauTree->Branch("againstMuonTight",&allTau_againstMuonTight,"againstMuonTight/I");
	_tauTree->Branch("againstElectronLoose",&allTau_againstElectronLoose,"againstElectronLoose/I");
	_tauTree->Branch("againstElectronMedium",&allTau_againstElectronMedium,"againstElectronMedium/I");
	_tauTree->Branch("againstElectronMVA",&allTau_againstElectronMVA,"againstElectronMVA/I");
	_tauTree->Branch("NsignalPFChargedHadrCands",&allTau_NsignalPFChargedHadrCands,"NsignalPFChargedHadrCands/I");
	_tauTree->Branch("NsignalPFGammaCands",&allTau_NsignalPFGammaCands,"NsignalPFGammaCands/I");
	_tauTree->Branch("leadPFChargedHadrCandPt",&allTau_leadPFChargedHadrCandPt,"leadPFChargedHadrCandPt/F");
	_tauTree->Branch("byLooseIsolation",&allTau_byLooseIsolation,"byLooseIsolation/F");
	_tauTree->Branch("byMediumIsolation",&allTau_byMediumIsolation,"byMediumIsolation/F");
	_tauTree->Branch("byTightIsolation",&allTau_byTightIsolation,"byTightIsolation/F");
	_tauTree->Branch("met",&allTau_met ,"met/F");
	_tauTree->Branch("metSig",&allTau_metSig ,"metSig/F");
	_tauTree->Branch("metPhi",&allTau_metPhi ,"metPhi/F");
	_tauTree->Branch("mc_pt",&allTau_mc_pt,"mc_pt/F");
	_tauTree->Branch("mc_eta",&allTau_mc_eta,"mc_eta/F");
	_tauTree->Branch("mc_phi",&allTau_mc_phi,"mc_phi/F");
	_tauTree->Branch("nearestJetDR",&allTau_nearestJetDR,"nearestJetDR/F");
	_tauTree->Branch("nearestJetPt",&allTau_nearestJetPt,"nearestJetPt/F");
	
	float truthTau_mass, truthTau_pt, truthTau_eta, truthTau_phi;
	float truthTau_nearestTauDR, truthTau_nearestTauPt, truthTau_nearestTauEta, truthTau_nearestTauPhi, truthTau_nearestTauMass;
	float truthTau_visibleMass, truthTau_visiblePt, truthTau_visibleEta, truthTau_visiblePhi;
	int truthTau_dau0, truthTau_dau1, truthTau_dau2;
	int truthTau_byLooseCombinedIsolationDeltaBetaCorr,truthTau_againstMuonTight,truthTau_againstElectronLoose,truthTau_decayModeFinding,truthTau_againstElectronMedium,truthTau_againstElectronMVA;
	int truthTau_NsignalPFChargedHadrCands, truthTau_NsignalPFGammaCands;
	float truthTau_leadPFChargedHadrCandPt, truthTau_byLooseIsolation, truthTau_byMediumIsolation, truthTau_byTightIsolation;
	
	
	TTree* _truthTauTree = new TTree("truthTauTree","truthTauTree");
	_truthTauTree->Branch("mass",&truthTau_mass,"mass/F");
	_truthTauTree->Branch("pt",&truthTau_pt,"pt/F");
	_truthTauTree->Branch("eta",&truthTau_eta,"eta/F");
	_truthTauTree->Branch("phi",&truthTau_phi,"phi/F");
	_truthTauTree->Branch("visibleMass",&truthTau_visibleMass,"visibleMass/F");
	_truthTauTree->Branch("visiblePt",&truthTau_visiblePt,"visiblePt/F");
	_truthTauTree->Branch("visibleEta",&truthTau_visibleEta,"visibleEta/F");
	_truthTauTree->Branch("visiblePhi",&truthTau_visiblePhi,"visiblePhi/F");
	_truthTauTree->Branch("nearestTauMass",&truthTau_nearestTauMass,"nearestTauMass/F");
	_truthTauTree->Branch("nearestTauPt",&truthTau_nearestTauPt,"nearestTauPt/F");
	_truthTauTree->Branch("nearestTauEta",&truthTau_nearestTauEta,"nearestTauEta/F");
	_truthTauTree->Branch("nearestTauPhi",&truthTau_nearestTauPhi,"nearestTauPhi/F");
	_truthTauTree->Branch("nearestTauDR",&truthTau_nearestTauDR,"nearestTauDR/F");
	_truthTauTree->Branch("decayModeFinding",&truthTau_decayModeFinding,"decayModeFinding/I");
	_truthTauTree->Branch("byLooseCombinedIsolationDeltaBetaCorr",&truthTau_byLooseCombinedIsolationDeltaBetaCorr,"byLooseCombinedIsolationDeltaBetaCorr/I");
	_truthTauTree->Branch("againstMuonTight",&truthTau_againstMuonTight,"againstMuonTight/I");
	_truthTauTree->Branch("againstElectronLoose",&truthTau_againstElectronLoose,"againstElectronLoose/I");
	_truthTauTree->Branch("againstElectronMedium",&truthTau_againstElectronMedium,"againstElectronMedium/I");
	_truthTauTree->Branch("againstElectronMVA",&truthTau_againstElectronMVA,"againstElectronMVA/I");
	_truthTauTree->Branch("NsignalPFChargedHadrCands",&truthTau_NsignalPFChargedHadrCands,"NsignalPFChargedHadrCands/I");
	_truthTauTree->Branch("NsignalPFGammaCands",&truthTau_NsignalPFGammaCands,"NsignalPFGammaCands/I");
	_truthTauTree->Branch("leadPFChargedHadrCandPt",&truthTau_leadPFChargedHadrCandPt,"leadPFChargedHadrCandPt/F");
	_truthTauTree->Branch("byLooseIsolation",&truthTau_byLooseIsolation,"byLooseIsolation/F");
	_truthTauTree->Branch("byMediumIsolation",&truthTau_byMediumIsolation,"byMediumIsolation/F");
	_truthTauTree->Branch("byTightIsolation",&truthTau_byTightIsolation,"byTightIsolation/F");
	_truthTauTree->Branch("dau0",&truthTau_dau0,"dau0/I");
	_truthTauTree->Branch("dau1",&truthTau_dau1,"dau1/I");
	_truthTauTree->Branch("dau2",&truthTau_dau2,"dau2/I");
		...
		for(ev.toBegin(); !ev.atEnd() ; ++ev, ++ievt)
			{
				...
				std::cout << "Event Run "<< EVENT.run << " Lumi Section " << EVENT.lumi << " json " << EVENT.json <<std::endl;
				...
				if(Verbage) std::cout << "Label for lhe = " << evt.getBranchNameFor(ev,"source") << std::endl;
				if( !((evt.getBranchNameFor(ev,"source")).empty()) ){
					evt.getByLabel(ev,"source");
					if(Verbage) std::cout << "LHEEventProduct found!" << std::endl;
					bool lCheck=false;
				...
				if(Verbage)std::cout << "lhe V pt = " << lheV_pt << std::endl;
				...
				//
				//
				//______________ TAU STUFF _______________________________________
				//
				//
				...
				if(isMC_) {
					...
					for(size_t i = 0; i < genParticles->size(); ++ i) {
						...
						TLorentzVector mcp4 = GENPTOLOR(p);
						if (abs(id)==15) {
							if (p.numberOfDaughters()==0 ||  p.daughter(0)->pdgId()==id) continue;
							int ndau = p.numberOfDaughters();
							if (p.numberOfDaughters() >= 2) {
								truthTau_dau0 = p.daughter(0)->pdgId();
								truthTau_dau1 = p.daughter(1)->pdgId();
								if (p.numberOfDaughters() >= 3) truthTau_dau2 = p.daughter(2)->pdgId();
								truthTau_mass = mcp4.M();
								truthTau_pt = mcp4.Pt();
								truthTau_eta = mcp4.Eta();
								truthTau_phi = mcp4.Phi();
								truthTau_nearestTauDR = 99.;
								truthTau_nearestTauPt = -99.;
								truthTau_nearestTauEta = -99.;
								truthTau_nearestTauPhi = -99.;
								truthTau_nearestTauMass = -99.;
								truthTau_decayModeFinding = -99;
								truthTau_byLooseCombinedIsolationDeltaBetaCorr = -99;
								truthTau_againstMuonTight = -99;
								truthTau_againstElectronLoose = -99;
								truthTau_againstElectronMedium = -99;
								truthTau_againstElectronMVA = -99;
								truthTau_NsignalPFChargedHadrCands = -99;
								truthTau_NsignalPFGammaCands = -99;
								truthTau_leadPFChargedHadrCandPt = -99.;
								truthTau_byLooseIsolation = -99.;
								truthTau_byMediumIsolation = -99.;
								truthTau_byTightIsolation = -99.;
								
								for (unsigned int j = 0 ; j < iEvent->tauInfo.size() ; j++) {
									float dr = iEvent->tauInfo[j].p4.DeltaR(mcp4);
									if (dr < truthTau_nearestTauDR) {
										truthTau_nearestTauDR = dr;
										truthTau_nearestTauPt = iEvent->tauInfo[j].p4.Pt();
										truthTau_nearestTauEta = iEvent->tauInfo[j].p4.Eta();
										truthTau_nearestTauPhi = iEvent->tauInfo[j].p4.Phi();
										truthTau_nearestTauMass = iEvent->tauInfo[j].p4.M();
										truthTau_decayModeFinding = (iEvent->tauInfo[j].decayModeFinding > 0.5);
										truthTau_byLooseCombinedIsolationDeltaBetaCorr = (iEvent->tauInfo[j].byLooseCombinedIsolationDeltaBetaCorr > 0.5);
										truthTau_againstMuonTight = (iEvent->tauInfo[j].againstMuonTight > 0.5);
										truthTau_againstElectronLoose = (iEvent->tauInfo[j].againstElectronLoose > 0.5);
										truthTau_againstElectronMedium = (iEvent->tauInfo[j].againstElectronMedium > 0.5);
										truthTau_againstElectronMVA = (iEvent->tauInfo[j].againstElectronMVA > 0.5);
										truthTau_NsignalPFChargedHadrCands = iEvent->tauInfo[j].NsignalPFChargedHadrCands;
										truthTau_NsignalPFGammaCands = iEvent->tauInfo[j].NsignalPFGammaCands;
										truthTau_leadPFChargedHadrCandPt = iEvent->tauInfo[j].leadPFChargedHadrCandPt;
										truthTau_byLooseIsolation = (iEvent->tauInfo[j].byLooseIsolation > 0.5);
										truthTau_byMediumIsolation = (iEvent->tauInfo[j].byMediumIsolation > 0.5);
										truthTau_byTightIsolation = (iEvent->tauInfo[j].byTightIsolation > 0.5);
									}
								}
								TLorentzVector visibleP = VisibleP(p);
								truthTau_visibleMass = visibleP.M();
								truthTau_visiblePt = visibleP.Pt();
								truthTau_visibleEta = visibleP.Eta();
								truthTau_visiblePhi = visibleP.Phi();
								if (ana.getParameter<bool>("verbose")) {
									std::cout << "Truth tau visible (pt,eta,phi,m)=("
									<< truthTau_visiblePt << ","
									<< truthTau_visibleEta <<","
									<< truthTau_visiblePhi <<","
									<< truthTau_visibleMass << ")" << std::endl;
				...
				for (unsigned int j = 0 ; j < iEvent->tauInfo.size() ; j++) {
					bool record = false;
					if (iEvent->tauInfo[j].p4.Pt() > 20. && cand->size() > 0) {
						record = true;
						for (unsigned int i = 0 ; i < cand->at(0).H.jets.size() ; i++) {
							if (cand->at(0).H.jets[i].p4.DeltaR(iEvent->tauInfo[j].p4) < 0.2) record = false;
						}
						for (unsigned int i = 0 ; i < cand->at(0).FatH.jets.size() ; i++){
							if (cand->at(0).FatH.jets[i].p4.DeltaR(iEvent->tauInfo[j].p4) < 0.2) record = false;
						}
					}
					if (record) {
						/*
						 allTau_nearestTruthDR = 99.;
						 for(size_t i = 0; i < genParticles->size(); ++ i) {
						 ...
						 int ndau = p.numberOfDaughters();
						 if (p.numberOfDaughters() >= 2) {
						 allTau_nearestTruthMass[nalltruthtau-1] = mcp4.M();
						 tau_nearestTruthPt[nalltruthtau-1] = mcp4.Pt();
						 tau_trutheta[nalltruthtau-1] = mcp4.Eta();
						 tau_truthphi[nalltruthtau-1] = mcp4.Phi();
						 tau_truthnearestTauDR[nalltruthtau-1] = 99.;
						 for (unsigned int j = 0 ; j < iEvent->tauInfo.size() ; j++) {
						 float dr = iEvent->tauInfo[j].p4.DeltaR(mcp4);
						 if (dr < truthTau_nearestAllTauDR[nalltruthtau-1]) {
						 truthTau_nearestTauDR[nalltruthtau-1] = dr;
						 truthTau_nearestTauPt[nalltruthtau-1] = iEvent->tauInfo[j].p4.Pt();
						 truthTau_nearestTauEta[nalltruthtau-1] = iEvent->tauInfo[j].p4.Eta();
						 truthTau_nearestTauPhi[nalltruthtau-1] = iEvent->tauInfo[j].p4.Phi();
						 truthTau_nearestTauMass[nalltruthtau-1] = iEvent->tauInfo[j].p4.M();
						 }
						 }
						 TLorentzVector visibleP = VisibleP(p);
						 truthTau_visibleMass[nalltruthtau-1] = visibleP.M();
						 truthTau_visiblePt[nalltruthtau-1] = visibleP.Pt();
						 truthTau_visibleEta[nalltruthtau-1] = visibleP.Eta();
						 truthTau_visiblePhi[nalltruthtau-1] = visibleP.Phi();
						 if (ana.getParameter<bool>("verbose")) {
						 std::cout << "Truth tau visible (pt,eta,phi,m)=("
						 << truthTau_visiblePt[nalltruthtau-1] << ","
						 << truthTau_visibleEta[nalltruthtau-1] <<","
						 << truthTau_visiblePhi[nalltruthtau-1] <<","
						 << truthTau_visibleMass[nalltruthtau-1] << ")" << std::endl;
						...
						allTau_mass = iEvent->tauInfo[j].p4.M() ;
						allTau_pt =iEvent->tauInfo[j].p4.Pt() ;
						allTau_eta =iEvent->tauInfo[j].p4.Eta() ;
						allTau_phi =iEvent->tauInfo[j].p4.Phi() ;
						allTau_decayModeFinding = (iEvent->tauInfo[j].decayModeFinding > 0.5);
						allTau_byLooseCombinedIsolationDeltaBetaCorr = (iEvent->tauInfo[j].byLooseCombinedIsolationDeltaBetaCorr > 0.5);
						allTau_againstMuonTight = (iEvent->tauInfo[j].againstMuonTight > 0.5);
						allTau_againstElectronLoose = (iEvent->tauInfo[j].againstElectronLoose > 0.5);
						allTau_againstElectronMedium = (iEvent->tauInfo[j].againstElectronMedium > 0.5);
						allTau_againstElectronMVA = (iEvent->tauInfo[j].againstElectronMVA > 0.5);
						allTau_NsignalPFChargedHadrCands = iEvent->tauInfo[j].NsignalPFChargedHadrCands;
						allTau_NsignalPFGammaCands = iEvent->tauInfo[j].NsignalPFGammaCands;
						allTau_leadPFChargedHadrCandPt = iEvent->tauInfo[j].leadPFChargedHadrCandPt;
						allTau_byLooseIsolation = (iEvent->tauInfo[j].byLooseIsolation > 0.5);
						allTau_byMediumIsolation = (iEvent->tauInfo[j].byMediumIsolation > 0.5);
						allTau_byTightIsolation = (iEvent->tauInfo[j].byTightIsolation > 0.5);
						
						allTau_met = iEvent->pfmet.p4.Pt();
						allTau_metPhi = iEvent->pfmet.p4.Phi();
						allTau_metSig = iEvent->pfmet.metSig;
						allTau_mc_pt = iEvent->tauInfo[j].mcFourMomentum.Pt();
						allTau_mc_eta = iEvent->tauInfo[j].mcFourMomentum.Eta();
						allTau_mc_phi = iEvent->tauInfo[j].mcFourMomentum.Phi();
						
						allTau_nearestJetDR = 9999.;
						allTau_nearestJetPt = 0.;
						for (unsigned int i = 0 ; i < iEvent->simpleJets2.size() ; i++) {
							float dr = iEvent->simpleJets2[i].p4.DeltaR(iEvent->tauInfo[j].p4);
							if (dr < allTau_nearestJetDR && dr > 0.2 && iEvent->simpleJets2[i].p4.Pt() > 20.) {
								allTau_nearestJetDR = dr;
								allTau_nearestJetPt = iEvent->simpleJets2[i].p4.Pt();
							}
						}
				...
				if(cand->size() == 0) 
					{
						if(Verbage)          std::cout << "W event loss due to tigther cuts" << std::endl;
						continue;
					}
				...
				aJetsFat.reset();
				if(Verbage)	std::cout << "Before Fat Jet Stuff" << std::endl;
				
				if(vhCand.FatH.FatHiggsFlag){ 
					if(Verbage)	std::cout << " FatJet == true? oh god"  << std::endl;
					...
					if(Verbage)	std::cout << "father jets set to "<<nfathFilterJets  << std::endl;
					...
					if(Verbage)	std::cout << "Almost done with FatJets"  << std::endl;
				...
				if(Verbage)	std::cout << " Right after fat jet stuff"  << std::endl;
				...
				if(vhCand.H.HiggsFlag){    
					...
					if(Verbage)	std::cout << " additional jets"  << std::endl;
					...
					if(Verbage)	std::cout << " Where is the bug"  << std::endl;
				...
				if(Verbage)	std::cout << "vector boson info"  << std::endl;
				...
				if(Verbage)	std::cout << " Before look at Met info"  << std::endl;
				...
				if(Vtype == VHbbCandidate::Zemu ){
					...
					if(Verbage) cout <<"electron pt " << vLeptons.pt[0] << " eta " << vLeptons.eta[0] << endl;
					...
					if (vLeptons.pt[1]>10&&vLeptons.pt[0]>10){
						...
						if( vLeptons.pt[1]>20&&vLeptons.pt[0]>20){
							...
							else if (weightEleHLTRecoAndId<weightMuonHLTRecoAndID)weightEleHLTRecoAndId = 1.0;
						}
				...
				if(Vtype == VHbbCandidate::Ztaumu ){
					...
					leptonForTop=vhCand.V.muons[0].p4;
					vLeptons.set(vhCand.V.muons[0],0,13,aux); 
					vLeptons.set(vhCand.V.taus[0],1,15,aux);
					nvlep=2;
					firstAddMu=1;
					firstAddTau=1;
				...
				Nevents_Wtaun++;
					leptonForTop=vhCand.V.taus[0].p4;
					vLeptons.set(vhCand.V.taus[0],0,15,aux); 
					nvlep=1;
					firstAddTau=1;
					weightTrig2012SingleEle = weightTrig2012ASingleEle;
					weightTrig2012 = weightTrig2012A;
				...
				if(Vtype == VHbbCandidate::Ztautau ){
					Nevents_Ztautau++;
					vLeptons.set(vhCand.V.taus[0],0,15,aux); 
					vLeptons.set(vhCand.V.taus[1],0,15,aux); 
					nvlep=2;
					firstAddTau=2;
				...
				if(fromCandidate)
					{
						...
						//for(size_t j=firstAddTau;j< vhCand.V.taus.size();j++) aLeptons.set(vhCand.V.taus[j],nalep++,15,aux);
					}
				else
					{
						...
						/* for(size_t j=0;j< iEvent->tauInfo.size();j++)
						 { 
						 if((j!= vhCand.V.firstLepton && j!= vhCand.V.secondLepton) || ((Vtype != VHbbCandidate::Wtaun) && (Vtype != VHbbCandidate::Ztaumu) && (Vtype != VHbbCandidate::Ztaue) ))
						 aLeptons.set(iEvent->tauInfo[j],nalep++,15,aux);
						 }*/
				...
				for (unsigned int i=0; i<aux.mcZ.size(); i++){
					...
					if(Verbage)std::cout << "Gen Z info " << endl;
				...
				if(Verbage)std::cout << "Gen Top info " << endl;
				...
				for (unsigned int i=0; i<aux.mcW.size(); i++){
					...
					if(Verbage)std::cout << "Gen Wstar info " << endl;
				...
				if(Verbage)std::cout << "Higgs Flag info " << endl;
				...
				if(Verbage)std::cout << "Before Fill outTree" << endl;
